fp.distance.from.boundary.analysis <- function(folder.path){
  ...
  data.fp <- mcmapply(function(colony, fp, location){
      ...
      df.f.tomato <- output.i.tomato %>% group_by(Subclone.ID, Subclone.Color) %>% summarize(D = min(D))
      df.f.ypet <- output.i.ypet %>% group_by(Subclone.ID, Subclone.Color) %>% summarize(D = min(D))
